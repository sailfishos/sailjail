...
clean:
	rm -fr $(BUILD_DIR)
	rm -f $(PERMISSIONS_DIR)/*~ $(SRC_DIR)/*~ $(INCLUDE_DIR)/*~ *~
	make -C unit clean
...
INSTALL_PERMISSIONS_DIR = $(DESTDIR)/etc/sailjail/permissions
INSTALL_INCLUDE_DIR = $(DESTDIR)/usr/include/sailjail
INSTALL_PKGCONFIG_DIR = $(DESTDIR)$(ABS_LIBDIR)/pkgconfig

install: $(INSTALL_BIN_DIR) $(INSTALL_PERMISSIONS_DIR)
	$(INSTALL_BIN) $(RELEASE_EXE) $(INSTALL_BIN_DIR)
	$(INSTALL_DATA) $(PERMISSIONS_DIR)/*.profile $(INSTALL_PERMISSIONS_DIR)
	$(INSTALL_DATA) $(PERMISSIONS_DIR)/*.permission $(INSTALL_PERMISSIONS_DIR)
